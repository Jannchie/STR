...
class STR(torch.nn.Module):
  ...
  def user_item_embedding(self, u: torch.Tensor) -> torch.Tensor:
    ...
    if aggregate == 'mean':
      ...
      mask = ie.sum(dim=-1) != 0
      weights = self.user_nei_item.values[u] ** self.config.get('aggregate_a', 0.75) * mask.float() 
      weights = weights / weights.sum(dim=1, keepdim=True)
  ...
  def forward(self, user: torch.Tensor, item: torch.Tensor, group: torch.Tensor = None):
    ...
    loss_h = self.get_loss(self.user_item_embedding(user), ie) if w_ui > 0 else torch.zeros_like(loss_ui)
    neg_loss = self.get_neg_loss(ue, neg_ie)
    return (loss_ui * w_cf + w_ui * loss_h).mean() + self.w_g * loss_g   + neg_loss
