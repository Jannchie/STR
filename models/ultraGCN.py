...
class BaseModel(torch.nn.Module):
    def __init__(self, train_df, test_df, users: int, items: int, hparams: HParams):
        ...
        self.u_d = torch.tensor(self.train_df.groupby(USER_COL).agg({ITEM_COL: 'count'}).values).view(-1)
        self.i_d = torch.tensor(self.train_df.groupby(ITEM_COL).agg({USER_COL: 'count'}).values).view(-1)
        ...
        self.user_embeds = torch.nn.Embedding(users, hparams.latent_factors, device=self.device)
        self.item_embeds = torch.nn.Embedding(items, hparams.latent_factors, device=self.device)
...
class UltraGCN(BaseModel):    
    ...
    def __init__(self, train_df, test_df,
        ...
        self.use_ii = False
        self.use_uu = True
        u, i, v = torch.tensor(self.train_df[USER_COL].values), torch.tensor(self.train_df[ITEM_COL].values), torch.tensor(self.train_df[SCORE_COL].values if SCORE_COL in self.train_df else np.ones(len(self.train_df)))
        self.sp_mat = torch.sparse.LongTensor(torch.stack((u, i)), v, torch.Size([self.users, self.items]))
        ...
        self.uu_mat = torch.sparse.mm(self.sp_mat.float(), self.sp_mat.t().float()).to(self.device)
    ...
    def init_ii(self, type, num_neighbors=10):
        self.ii_nei_idx = torch.zeros((self.items, num_neighbors), dtype=torch.long, device=self.device)
        self.ii_nei_val = torch.zeros((self.items, num_neighbors), dtype=torch.float, device=self.device)
        ...
        for i in tqdm(range(idx_length), total=idx_length):
            ...
            oi = torch.mul(self.ii_constraint_mat['beta_id'][i], self.ii_constraint_mat['beta_id'])
            row = oi * di
    ...
    def get_omegas(self, users: torch.Tensor, items: torch.Tensor):
        if self.w2 > 0:
            weight = torch.mul(self.ui_constraint_mat['beta_ud'][users], self.ui_constraint_mat['beta_id'][items])
    ...
    def forward(self, users: torch.Tensor, items: torch.Tensor):
        ...
        iv = self.item_vec(items)
        if (self.neg_num and self.training):
            neg_items = torch.randint(self.items, (items.shape[0]*self.neg_num,), device=self.device)
    ...
    def get_loss(self, users, items, scores):
        ...
        neg_loss = self.criterion(y_neg_pred, scores_neg).reshape(self.neg_num,-1) * neg_omegas.reshape(self.neg_num,-1)
